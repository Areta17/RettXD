...
return buffer
	...
	const fake = {
	 ...
	 { remoteJid: "6283851929130-1589321480@g.us" } : {}) 
                },
	 message: { 
		"extendedTextMessage": {
	...
	const fgclink = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6283851929130-1589321480@g.us","inviteCode": "m","groupName": "Rett Bot", "caption": `© Margareta`, 'jpegThumbnail': global.thumb}}}
    const fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283851929130-1589321480@g.us" } : {}) },message: { "videoMessage": { "title":`*AUTO DOWNLOAD AUDIO YOUTUBE*`, "h": `Hmm`,'seconds': '10000000⁰0', 'caption': `*AUTO DOWNLOAD AUDIO YOUTUBE*`, 'jpegThumbnail': global.thumb}}}

		//TEMPLATE BUTTON!
		const buttonsDefault = [
			//{ callButton: { displayText: `Call Owner!`, phoneNumber: `+6285849261085` } },
			{ urlButton: { displayText: `Instagram!`, url : `https://instagram.com/rrta.xy` } },
...
switch(command){
			case'menu':
			case'help': {
				footerr = `📮Jika menemukan bug, segera hubungi owner!\n© 2022 Margareta`
				anu = `${fke}Hi ${m.pushName} 👋, i'am Rett Bot!${fke}
...
`,
					btn = [{
                                urlButton: {
                                    ...
                                    url: 'https://instagram.com/rrta.xy'
                                }
                            }, {
                                urlButton: {
                                    displayText: 'ɢʀᴏᴜᴘ ᴡʜᴀᴛꜱᴀᴘᴘ',
                                    url: 'https://chat.whatsapp.com/FSK0yUVmBn791WlgxYwUo4'
                                }
                            }, {
                                quickReplyButton: {
                                    displayText: '👤 ᴏᴡɴᴇʀ',
				...
				conn.sendMessage(from, {text : `*This is an ad*\n\n•───────────•\n${jsn[0].setting}\n•───────────•\n\n_© 2022 rrta.xy.com_`}, {quoted :{
				key: { fromMe: false,
				participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283851929130-1589321480@g.us" } : {})},
...
> Fitur antilink, antivirus, antiyt,,,,,
> Bisa hidetag/tagall

*📝 Note :*
> Jika mau sewa, hubungi owner bot
...
Konfirmasi pembayaran mu dengan menunjjukan bukti transfer kepada owner!
            ...
            case 'public': {
                ...
                naze.public = true
                m.reply('*Successful use for public*')
            }
            break
            case 'self': {
                if (!isCreator) throw mess.owner
                naze.public = false
                m.reply('*Successful use for personal*')
            }
            break
			case 'tes':{
			...
			case 'tk': case 'tarikkomisi': {
				...
				let nyobb = list
				conn.sendMessage(from, {text:`*PILLIH E-WALLET*\nPilih jenis wallet yang ingin anda gunakan!`, title: 'WALLET', footer: '© 2022 Margareta', buttons: nyobb})
				}
				break
			...
			case 'ytplay': {
                ...
                let buttons = [
                	{ urlButton: { displayText: `𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐔𝐬!`, url : `https://instagram.com/rrta.xy` } },
...
⊳ Url : ${anu.url}`,
			...
			case 'dariytskunci':{ //KUNCI = BIAR GA DR AKSES HEHE
                ...
                conn.sendMessage(from, { caption: `⭔ Title : ${resmed.title}\n⭔ File Size : ${resmed.filesizeF}\n⭔ Url : ${isUrl(text)}\n⭔ Ext : MP3\n⭔ Resolusi : ${args[1] || '128kbps'}`, image: {url: resmed.thumb}, templateButtons: btnYts, footer: `© 2022 rrta.xy.com` })
				}
				break
...
break*/
			...
			let buttons = [
                ...
                conn.sendMessage(from, {text:`*NGETES BUTTON*`, title: 'WALLET', footer: '© 2022 Margareta', buttons: buttons})
                }
                break
			case 'request': case 'requestfitur':{
				if (!text) return m.reply(`Text mana?\n\nExample : ${prefix + command} Tambahin fitur tiktok downloader dong`)
				conn.sendMessage(`6283851929130@s.whatsapp.net`, {text: `*Hallo owner ada yang request fitur nih*\n\nPesan : ${text}\nPengirim : ${m.sender}`}, {quoted: {key: { fromMe: false,
				participant: `${m.sender}`, ...(from ? { remoteJid: "6283851929130-1589321480@g.us" } : {})},
				message: { "extendedTextMessage": {
				"text": `Request Fitur!`,
				"title": `Pasang iklan murah bang`,
				'jpegThumbnail': global.thumb}}}})
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
           ...
           case 'setppbot': {
	        ...
	        case 'tourl': {
          	  ...
          	  break
			case 'name':
				conn.sendMessage(from, {text: conn.getName('6283851929130@s.whatsapp.net')})
				break
//++++++++++++++ END CASE +++++++++++++++
default:
...
if ((m.mtype === 'reactionMessage') && m.isGroup) {
    conn.sendMessage(from, {text: `Terdeteksi *${m.pushName} @${m.sender.split("@")[0]}* Mengirim Reaction.`, mentions: [m.sender]}, {quoted: fake})
        ...
        if ((m.mtype === 'groupInviteMessage' || budy.startsWith('https://chat') || budy.startsWith('Buka tautan ini')) && !m.isBaileys && !m.isGroup) {
			...
			conn.sendMessage(from, {text:`INVITE DETECT\n\nMau masukin bot ini ke group kamu?`, title: 'LINK GROUP DECTETED', footer: '© 2022 Margareta', buttons: buttons, headerType: 5})
		}
...
print(e)
conn.sendMessage(`6283851929130@s.whatsapp.net`, { text: String(e) }, { quoted: m})
}
}
}
} catch(e) {
